...
@Component({
  selector: 'app-explore-container-weight',
  templateUrl: './explore-container.component.html',
  styleUrls: ['./explore-container.component.scss'],
})
export class WeightExploreContainerComponent implements OnInit {
  modal: HTMLElement;
  registers$= of([]);
  ...
  async presentModal() {
    const modal = await this.modalController.create({
      component: WeigthModalComponent
    });
  ...
  updateTable(){
    this.registers$= of([]);
    this.registers$ = this.registerService.searchWeightByName(this.authService.getName());
  }
}
